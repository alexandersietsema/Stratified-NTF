#tensorly_ntf.py
"""
This is a version of Tensorly's non_negative_parafac function, altered to
use handle different input data, use a different loss function, and return
different values.
"""
...
from utils import my_reshape, batch_tensor_product
import numpy as np

def loss(A,w,h):
    B = batch_tensor_product([w, *h]).sum(0)
    out = np.linalg.norm(A - B) ** 2
    # TODO: Consider rescaling based on the number of data points
...
def non_negative_parafac(
    ...
    # allow for providing initial iterates
    if initial:
        weights, factors = initial
    weights_init, factors_init = weights.copy(), factors.copy()
    
    loss_val = loss(tensor, factors[0].T, [factors[i].T for i in range(1,len(factors))])
    rec_errors = []
    rec_errors.append(loss_val)
    ...
    for iteration in tqdm(range(n_iter_max)):
        ...
        for mode in modes_list:
            ...
            """
            print("="*10, "NTF ", iteration, " ", mode, " ", "-"*10)
            print(numerator)
            print("="*30)
            """
        ...
        if tol:
            ...
            rec_errors.append(loss(tensor, factors[0].T, [factors[i].T for i in range(1,len(factors))]))
    ...
    if return_errors:
        if return_initial:
            return cp_tensor, rec_errors, weights_init, factors_init, weights, factors
        return weights, factors, rec_errors
    else:
        if return_initial:
            return cp_tensor, weights_init, factors_init
        return cp_tensor
